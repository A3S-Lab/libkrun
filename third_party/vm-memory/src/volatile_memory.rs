...
impl PtrGuard {
    #[cfg(unix)]
    const READ_PROT: i32 = libc::PROT_READ;
    #[cfg(not(unix))]
    const READ_PROT: i32 = 0;

    #[cfg(unix)]
    const WRITE_PROT: i32 = libc::PROT_WRITE;
    #[cfg(not(unix))]
    const WRITE_PROT: i32 = 0;
    ...
    fn read(mmap: Option<&MmapInfo>, addr: *mut u8, len: usize) -> Self {
        Self::new(mmap, addr, Self::READ_PROT, len)
    }
...
impl PtrGuardMut {
    fn write(mmap: Option<&MmapInfo>, addr: *mut u8, len: usize) -> Self {
        Self(PtrGuard::new(mmap, addr, PtrGuard::WRITE_PROT, len))
    }
