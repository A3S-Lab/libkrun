...
impl WriteVolatile for Stdout {
    fn write_volatile<B: BitmapSlice>(
        ...
        #[cfg(windows)]
        {
            ...
            let src = buf.subslice(0, buf.len())?;
            // SAFETY: tmp is valid for writes and src is a valid volatile slice.
            let copied = unsafe { copy_from_volatile_slice(tmp.as_mut_ptr(), &src, tmp.len()) };
            self.write(&tmp[..copied])
...
impl ReadVolatile for std::fs::File {
    fn read_volatile<B: BitmapSlice>(
        ...
        let mut tmp = vec![0_u8; buf.len()];
        let bytes = self.read(&mut tmp).map_err(VolatileMemoryError::IOError)?;
        let dst = buf.subslice(0, bytes)?;
        // SAFETY: dst points to valid guest memory for `bytes`, tmp has at least `bytes` bytes.
        unsafe {
            copy_to_volatile_slice(&dst, tmp.as_ptr(), bytes);
        }
        Ok(bytes)
    }
}
...
impl WriteVolatile for std::fs::File {
    fn write_volatile<B: BitmapSlice>(
        ...
        let mut tmp = vec![0_u8; buf.len()];
        let src = buf.subslice(0, buf.len())?;
        // SAFETY: tmp is valid for writes and src is valid volatile memory.
        let copied = unsafe { copy_from_volatile_slice(tmp.as_mut_ptr(), &src, tmp.len()) };
        self.write(&tmp[..copied])
            .map_err(VolatileMemoryError::IOError)
    }
